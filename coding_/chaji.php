<?php include('head.php'); ?>
	<?php include('nav.php'); ?>
	<div id="contents" class="food">
		<div class="kv-same">
  		<img class="pc" src="./images/pic-chaji-kv.png" alt="お茶時">
  		<img class="sp" src="./images/pic-chaji-kv-sp.png" alt="お茶時">
  	</div>
		<img src="./images/icon-same.png" alt="アイコン" class="icon-same pc">
		<img src="./images/txt-chaji-caption.png" alt="お料理" class="caption pc">
		<p class="caption-p">茶事は一年を通じ、自然の移り変わりや季節に応じて催されます。新しい茶を使い<br />始める初冬の時期の「口切」の茶から、「古茶」と呼ぶ</p>
		<div class="media clearfix">
			<div class="media-img">
   			<img src="./images/pic-chaji-img-01.png" alt="">
			</div>
			<div class="media-text">
				<img class="pc" src="./images/border-same.png" alt="">
				<h5>展望大浴場金殿について</h5>
				<img src="./images/border-same.png" alt="">
				<p>茶事は一年を通じ、自然の移り変わりや季節に応じて催されます。新しい茶を使い始める初冬の時期の「口切」の茶から、「古茶」と呼ぶ前年の茶の残りを心おしみつつ使う晩秋の(名残の)茶まで、季節に応ずる「茶事」として「茶事」は一年を通じ、自然の移り変わりや季節に応じて催されます。新しい茶を使い始める初冬の時期の「口切」の茶から、「古茶」と呼ぶ前年の茶の残りを心おしみつつ使う晩秋の(名残の)茶まで、季節に応ずる。</p>
			</div>
		</div>
		<div class="media clearfix">
			<div class="media-img">
   			<img src="./images/pic-chaji-img-02.png" alt="">
			</div>
			<div class="media-text">
				<img class="pc" src="./images/border-same.png" alt="">
				<h5>展望大浴場金殿について</h5>
				<img src="./images/border-same.png" alt="">
				<p>茶事は一年を通じ、自然の移り変わりや季節に応じて催されます。新しい茶を使い始める初冬の時期の「口切」の茶から、「古茶」と呼ぶ前年の茶の残りを心おしみつつ使う晩秋の(名残の)茶まで、季節に応ずる「茶事」として「茶事」は一年を通じ、自然の移り変わりや季節に応じて催されます。新しい茶を使い始める初冬の時期の「口切」の茶から、「古茶」と呼ぶ前年の茶の残りを心おしみつつ使う晩秋の(名残の)茶まで、季節に応ずる。</p>
			</div>
		</div>
		<div class="media clearfix">
			<div class="media-img">
   			<img src="./images/pic-chaji-img-03.png" alt="">
			</div>
			<div class="media-text">
				<img class="pc" src="./images/border-same.png" alt="">
				<h5>展望大浴場金殿について</h5>
				<img src="./images/border-same.png" alt="">
				<p>茶事は一年を通じ、自然の移り変わりや季節に応じて催されます。新しい茶を使い始める初冬の時期の「口切」の茶から、「古茶」と呼ぶ前年の茶の残りを心おしみつつ使う晩秋の(名残の)茶まで、季節に応ずる「茶事」として「茶事」は一年を通じ、自然の移り変わりや季節に応じて催されます。新しい茶を使い始める初冬の時期の「口切」の茶から、「古茶」と呼ぶ前年の茶の残りを心おしみつつ使う晩秋の(名残の)茶まで、季節に応ずる。</p>
			</div>
		</div>
<?php include('footer.php'); ?>